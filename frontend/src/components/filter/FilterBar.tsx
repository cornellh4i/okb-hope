...
import FilterModal from "./FilterModal";
import okb_colors from '@/colors';
import FilterUserTable from "./FilterUserTable"; // <-- Import FilterUserTable here
    ...
    useEffect(() => {
        const applyFilter = () => {
            const filtered = userList.filter(user => {
                return (
                    (ageGroup ? user.ageGroup === ageGroup : true) &&
                    (gender ? user.gender === gender : true) &&
                    (condition ? user.condition === condition : true)
                );
            });
        ...
        applyFilter();
    }, [ageGroup, gender, condition, userList]);
        ...
        <div className="flex flex-row justify-center items-center gap-2 mx-36">
            ...
            <div className="h-12 px-6 py-3 bg-white rounded-lg border border-zinc-600 justify-between items-center inline-flex">
                <div className="dropdown">
                    ...
                    <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                        {ageGroups.map(e => (
                            <li key={e} onClick={() => setAgeGroup(e)}>{e}</li>
                        ))}
                    </ul>
                </div>
            </div>
            ...
            <div className="h-12 px-6 py-3 bg-white rounded-lg border border-zinc-600 justify-between items-center inline-flex">
                <div className="dropdown">
                    ...
                    <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                        {genders.map(e => (
                            <li key={e} onClick={() => setGender(e)}>{e}</li>
                        ))}
                    </ul>
                </div>
            </div>
            ...
            <div className="h-12 px-6 py-3 bg-white rounded-lg border border-zinc-600 justify-between items-center inline-flex">
                <div className="dropdown">
                    ...
                    <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                        {conditions.map(e => (
                            <li key={e} onClick={() => setCondition(e)}>{e}</li>
                        ))}
                    </ul>
                </div>
            </div>

            <button className="px-8 py-3 bg-white rounded-2xl border border-sky-700 justify-center items-center gap-2.5 inline-flex" onClick={() => setFilteredUsers(filteredUsers)}>
                <div className="text-sky-700 text-base font-bold text-center">Filter</div>
            </button>
            ...
            {/* Render FilterUserTable with the filtered users */}
            <FilterUserTable currentRecords={filteredUsers} onDelete={onDelete} selectedUsers={setSelectedUserIds} />
        </div>
    );
};
