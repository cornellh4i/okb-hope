          ...
          {!user &&
            <li className={`${router.pathname === '/discover' ? 'underline-offset-1 custom-active' : ''}`}>
              ...
              <Link href={`/discover`} className="w-full">
                <div className="text-right text-[18px] text-[#195BA5] font-montserrat font-medium">Discover Professionals</div>
              </Link>
            </li>}
          {user && user.userType == "patient" &&
                ...
                <ul tabIndex={0} className="menu dropdown-content p-2 shadow bg-base-100 rounded-box w-52">
                  <div style={{ width: '100%', height: '100%', paddingLeft: 16, paddingRight: 16, paddingTop: 8, paddingBottom: 8, background: 'white', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: 48, display: 'inline-flex' }}>
                    {user.userType == "patient" ?
                      <><Link href={`/patient/${user.uid}/edit_profile`} className={`text-[18px] font-normal font-montserrat`} style={{ color: '#195BA5', wordWrap: 'break-word' }}>Edit Profile</Link>
                        <Link href={`/patient/${user?.uid}/report_history`} className={`text-[18px] font-normal font-montserrat`} style={{ color: '#195BA5', wordWrap: 'break-word' }}>Report History</Link></> : user.userType == "psychiatrist" ?
                        <Link href={`/psychiatrist/${user.uid}/edit_psych`} className={`text-[18px] font-normal font-montserrat`} style={{ color: '#195BA5', wordWrap: 'break-word' }}>Edit Profile</Link> : <></>}
                    <div onClick={logout} style={{ justifyContent: 'center', alignItems: 'center', display: 'inline-flex', gap: '15px', cursor: 'pointer' }}>
                      <div className={`text-[18px] font-normal font-montserrat`} style={{ textAlign: 'center', color: '#A52119', wordWrap: 'break-word' }}>Log Out</div><Logout />
                    </div>
                  </div>
                </ul>
              </div> :
