...
//         const approvedPsychiatrists = fetchedPsychiatrists.filter(
//           (psychiatrists) => psychiatrists.status === "approved");
...
useEffect(() => {
 async function fetchData() {
   try {
     const fetchedPsychiatrists: IPsychiatrist[] = await fetchAllProfessionals();
     const approvedPsychiatrists = await Promise.all(
       fetchedPsychiatrists.map(async (psych) => {
         const isApproved = await fetchPsychiatristStatus(psych.uid);
         if (isApproved) {
           return { ...psych, status: 'approved' }; // Add 'approved' status if the psychiatrist is approved
         }
         return null;
       })
     );
     const filteredPsychiatrists = approvedPsychiatrists.filter(psych => psych !== null);
...
const fetchPsychiatristStatus = async (psychiatristUID: string) => {
 ...
 if (docSnap.exists()) {
   const data = docSnap.data();
   const status = data.status;
   return status === "approved"; // Return true if the status is 'approved'
 } else {
   console.log(`No such psychiatrist document for ID: ${psychiatristUID}`);
